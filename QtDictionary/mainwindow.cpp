    ...
    ui(new Ui::MainWindow)
{
    ...
    //2010年大学英语四级词汇.txt作为输入和保存的文本
    myDictionary=new Dictionary("txt/2010年大学英语四级词汇.txt");
...
void MainWindow::on_actionRemember_triggered()
{
    ...
    qsrand(time.msec()+time.second()*1000);
     i = qrand() % myDictionary->Englishwords.size();    //产生的随机数
...
void MainWindow::on_ButtonSure_clicked()
{
    ...
    if(answer==myDictionary->Englishwords[i])
    {
      //  qDebug()<<"正确";
        ...
        myPlayer->setMedia(QUrl::fromLocalFile("music/正确.mp3"));
        myPlayer->play();
        this->on_actionRemember_triggered();
    }else
    {
        QMediaPlayer *myPlayer= new QMediaPlayer;
        myPlayer->setMedia(QUrl::fromLocalFile("music/错误.mp3"));
        myPlayer->play();
    }
